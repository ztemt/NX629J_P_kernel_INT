...
static int tcs3707_init_client(struct tcs3707_ctrl_t *t_ctrl)
{
	...
	usleep_range(1200, 1200 + 1);
	//msleep(500);
	do_gettimeofday(&start_tv);
	do {
			res = tcs3707_cci_i2c_read(t_ctrl, 0x92, &id);

			time_over = is_time_over(&start_tv, 500);
			if (!time_over)
				msleep(1);
		} while (!time_over);
...
static ssize_t tcs3707_read(struct file* file, char __user* buf, size_t count, loff_t* f_pos)
{
    ...
    else {//if missing > 0, means missing bytes not copied
...
static int32_t tcs3707_platform_remove(struct platform_device *pdev)
{
	...
	if (tcs3707_ctrl->power_supply) {
		//rc = cam_soc_util_regulator_disable(tcs3707_ctrl->power_supply, "laser", 2800000, 2800000, 80000, 0);
		//rc = regulator_disable(tcs3707_ctrl->cci_supply);
		if (rc)
			TCS_ERR("reg disable failed. rc=%d\n",rc);
	}
