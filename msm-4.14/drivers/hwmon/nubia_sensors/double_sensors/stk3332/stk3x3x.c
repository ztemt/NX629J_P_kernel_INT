...
#define PS_THRESH_DATA_MIN              75
#define PS_THRESH_DATA_MAX              1500
...
static int32_t stk3x3x_ps_calibration_thres(struct stk3x3x_data *ps_data){
    ...
    while (counter < PS_CALI_COUNT)
    {
        msleep(60);
        reading = stk3x3x_get_ps_reading(ps_data);
        SENSOR_LOG_INFO("%s: ps%d = %d\n", counter, reading);
        ps_sum += reading;
        counter++;
    }

    ps_data->cali_near = (ps_sum / PS_CALI_COUNT);
	if((ps_data->cali_near > PS_THRESH_DATA_MAX) || (ps_data->cali_near < PS_THRESH_DATA_MIN))
		return -2;
				...
				const char *buf, size_t size)
{
	...
	struct stk3x3x_data *ps_data =  dev_get_drvdata(dev);
	rc = kstrtoint(buf, 0, &val);
	ps_data->prox_debug = val & 0xff;
	if (ps_data->prox_debug) {

		disable_irq(ps_data->irq);
		cancel_work_sync(&ps_data->stk_work);
...
static int stk3x3x_set_wq(struct stk3x3x_data *ps_data)
{
	...
	ps_data->ps_cali_delay = ns_to_ktime(40 * NSEC_PER_MSEC);
