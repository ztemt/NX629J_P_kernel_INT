	...
	size_t count)
{
	...
	if(swc_val)
	{
		...
		if(ret)
		{
			dev_err(dev, "cs_press power on failed!\n");
			return ret;
		}
		g_power_stat = !!swc_val;
	}else
	{
		ret = regulator_disable(power_3v3);
		if(ret)
		{
			dev_err(dev, "cs_press power down failed!\n");
			return ret;
		}
		g_power_stat = 0;
	}
	LOG("Set cs_press power status to %d.\n", g_power_stat);
	return count;
}
...
void report_key(void)
{
	...
	stat = get_key_stat();
	event = get_key_event();
//	LOG("Check key stat_0x20:%d event_0xD3:%d\n", stat, event);
	...
	if((event & 0x20) == 0x20){
		key_press = get_key_pressure(IIC_LEFT_KEY_PRESS);
		input_report_abs(cs_input_dev, ABS_X, key_press);
		input_sync(cs_input_dev);
	}
	if((event & 0x40) == 0x40){
		key_press = get_key_pressure(IIC_RIGHT_KEY_PRESS);
		input_report_abs(cs_input_dev, ABS_Y, key_press);
		input_sync(cs_input_dev);
	}
