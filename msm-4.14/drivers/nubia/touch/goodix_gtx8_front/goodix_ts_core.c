...
int goodix_ts_gpio_power_on_off(struct goodix_ts_core *core_data, bool on)
{
	struct goodix_ts_board_data *ts_bdata = 
			board_data(core_data);
	if(on) {
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (r < 0)
		goto out;
	r = goodix_ts_gpio_power_on_off(core_data,true);
	if (r)
		goto out;
	...
	/*create sysfs files*/
	goodix_front_ts_sysfs_init(core_data);
